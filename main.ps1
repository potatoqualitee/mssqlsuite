...
if ("sqlcmd" -in $Install) {
   ...
   (Invoke-WebRequest https://packages.microsoft.com/keys/microsoft.asc).Content | sudo apt-key add -
   (Invoke-WebRequest https://packages.microsoft.com/config/ubuntu/20.04/prod.list).Content | sudo tee /etc/apt/sources.list.d/msprod.list
   sudo apt-get update
